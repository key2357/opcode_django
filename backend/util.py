...
from backend.config import SENSITIVE_FUNC


# 生成opcode_example csv格式  依赖读取opcode_example_result
def generate_opcode_csv(aliuid, uuid, file_md5):
    # 加一个字段order
    cursor = connection.cursor()
    cursor.execute("select uuid, file_md5, `name`, `caller`, argc, argv, `return`, `index`, dynamic "
                   "from malware_op_code where file_md5 = '{0}' "
                   "and uuid = '{1}' and aliuid = '{2}'".format(file_md5, uuid, aliuid))
